...
const PublicMediaKit = () => {
  ...
  const fetchMediaKit = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/media-kit/public/${username}`);
      const data = await response.json();

      if (data.success && data.media_kit) {
        setMediaKit(data.media_kit);
      } else {
  ...
  if (loading) {
    return (
      <LoadingContainer>
        <Spin size="large" />
        <LoadingText>Loading media kit...</LoadingText>
      </LoadingContainer>
    );
  }
  ...
  if (error || !mediaKit) {
    return (
      <ErrorContainer>
        ...
        <BackLink to="/">Go to NewCollab</BackLink>
      </ErrorContainer>
    );
  }
  ...
  return (
    <>
      <Helmet>
        <title>{mediaKit.display_name} - Media Kit | NewCollab</title>
        <meta name="description" content={mediaKit.tagline || `${mediaKit.display_name}'s media kit on NewCollab`} />
        <meta property="og:title" content={`${mediaKit.display_name} - Media Kit`} />
        <meta property="og:description" content={mediaKit.tagline} />
        {mediaKit.profile_photo_url && <meta property="og:image" content={mediaKit.profile_photo_url} />}
        <meta property="og:url" content={`https://newcollab.co/kit/${username}`} />
      ...
      <PageContainer>
        <ContentWrapper>
          ...
          <Footer>
            ...
            <FooterLogo to="/">NewCollab</FooterLogo>
          </Footer>
        </ContentWrapper>
      </PageContainer>
    </>
