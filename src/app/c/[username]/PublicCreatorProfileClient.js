...
import { FaBaby, FaGraduationCap, FaBriefcase, FaUserTie, FaGamepad, FaRunning, FaTshirt, FaLaptop, FaUtensils, FaPlane, FaDog, FaTools, FaMusic, FaBook, FaLeaf, FaGem, FaShoppingCart, FaInstagram, FaYoutube, FaTwitter, FaFacebook, FaCamera, FaTiktok, FaSnapchat, FaLinkedin, FaPinterest, FaTwitch, FaHeartbeat, FaHome, FaPaw, FaTv, FaFutbol, FaPaintBrush, FaUserFriends, FaChartLine, FaCar, FaLink, FaFacebookF, FaWhatsapp, FaLinkedinIn, FaGift } from 'react-icons/fa';
...
function getContentFormatBadge(format) {
  ...
  const formatStyles = {
    'reels': { label: 'Reels', icon: <FiVideo style={{ fontSize: 14 }} />, bgColor: 'linear-gradient(135deg, #833AB4 0%, #FD1D1D 50%, #FCB045 100%)', textColor: '#fff', borderColor: 'transparent' },
    'reel': { label: 'Reels', icon: <FiVideo style={{ fontSize: 14 }} />, bgColor: 'linear-gradient(135deg, #833AB4 0%, #FD1D1D 50%, #FCB045 100%)', textColor: '#fff', borderColor: 'transparent' },
    'video': { label: 'Video', icon: <FiVideo style={{ fontSize: 14 }} />, bgColor: '#f0f9ff', textColor: '#0369a1', borderColor: '#bae6fd' },
    'stories': { label: 'Stories', icon: <FiCamera style={{ fontSize: 14 }} />, bgColor: 'linear-gradient(135deg, #833AB4 0%, #E1306C 100%)', textColor: '#fff', borderColor: 'transparent' },
    'story': { label: 'Stories', icon: <FiCamera style={{ fontSize: 14 }} />, bgColor: 'linear-gradient(135deg, #833AB4 0%, #E1306C 100%)', textColor: '#fff', borderColor: 'transparent' },
    'live': { label: 'Live', icon: <MdOutlineLiveTv style={{ fontSize: 14 }} />, bgColor: '#fee2e2', textColor: '#dc2626', borderColor: '#fecaca', pulse: true },
    'short': { label: 'Shorts', icon: <FiFilm style={{ fontSize: 14 }} />, bgColor: '#fef3c7', textColor: '#d97706', borderColor: '#fde68a' },
    'shorts': { label: 'Shorts', icon: <FiFilm style={{ fontSize: 14 }} />, bgColor: '#fef3c7', textColor: '#d97706', borderColor: '#fde68a' },
    'podcast': { label: 'Podcast', icon: <MdOutlineAudiotrack style={{ fontSize: 14 }} />, bgColor: '#e0e7ff', textColor: '#4338ca', borderColor: '#c7d2fe' },
    'audio': { label: 'Audio', icon: <MdOutlineAudiotrack style={{ fontSize: 14 }} />, bgColor: '#e0e7ff', textColor: '#4338ca', borderColor: '#c7d2fe' },
    'post': { label: 'Post', icon: <FiImage style={{ fontSize: 14 }} />, bgColor: '#f0fdf4', textColor: '#166534', borderColor: '#bbf7d0' },
    'static': { label: 'Post', icon: <FiImage style={{ fontSize: 14 }} />, bgColor: '#f0fdf4', textColor: '#166534', borderColor: '#bbf7d0' }
  };

  let style = formatStyles['post'];
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, background: style.bgColor, color: style.textColor,
      border: `1px solid ${style.borderColor}`, fontSize: 12, fontWeight: 600,
...
export default function PublicCreatorProfileClient({ username }) {
  ...
  const handleBidChange = (id, value) => {
    ...
    setBidErrors(errors => ({ ...errors, [id]: undefined }));
  };
  ...
  if (error) return (
    <LandingPageLayout>
      <div style={{ textAlign: 'center', marginTop: 64, color: TEXT_COLOR, padding: '0 16px' }}>
        ...
        <button
          onClick={() => router.push('/')}
          style={{
            background: BRAND_COLOR, color: '#fff', border: 'none', borderRadius: 8,
            padding: '10px 22px', fontWeight: 700, fontSize: 16, cursor: 'pointer',
  ...
  if (loading || !profile) return (
    <LandingPageLayout>
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
        <div style={{ textAlign: 'center', color: SUBTLE_TEXT }}>Loading...</div>
      </div>
    </LandingPageLayout>
  );
  ...
  const getSocialIcon = (platform) => {
    ...
    if (platformLower.includes('tiktok')) return <FaTiktok style={{ color: '#000000', fontSize: 18 }} />;
    if (platformLower.includes('facebook')) return <FaFacebook style={{ color: '#1877F2', fontSize: 18 }} />;
    if (platformLower.includes('twitter') || platformLower.includes('x')) return <FaTwitter style={{ color: '#1DA1F2', fontSize: 18 }} />;
    if (platformLower.includes('linkedin')) return <FaLinkedin style={{ color: '#0A66C2', fontSize: 18 }} />;
  ...
  return (
    <LandingPageLayout>
      <div style={{ background: BG_COLOR, minHeight: '100vh', padding: 0 }}>
        ...
        {/* Cover Section */}
        <div style={{
          width: '100%', height: COVER_HEIGHT, background: COVER_GRADIENT,
          ...
          {profile && (
            <img
              ...
              style={{
                ...
                border: '4px solid #fff', boxShadow: '0 4px 24px rgba(16,185,129,0.18)',
        ...
        <div style={{
          maxWidth: 680, margin: '0 auto', padding: '72px 16px 32px 16px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 8, marginBottom: 16 }}>
            ...
            {profile.followers_count && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                ...
                {profile.projects && profile.projects.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: TEXT_COLOR }}>{profile.projects.length}</div>
                ...
                {socialLinksArr.map((link, idx) => {
                  ...
                  return (
                    <a
                      ...
                      style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: 6, background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', border: '1.5px solid #e5e7eb', width: 32, height: 32 }}
                    >
            ...
            {/* Trust Signal */}
            <div style={{ display: 'flex', gap: 12, marginTop: 8, marginBottom: 4, flexWrap: 'wrap', justifyContent: 'center', fontSize: 13, color: SUBTLE_TEXT }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ color: BRAND_COLOR }}>✓</span> Verified Creator</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><span style={{ color: BRAND_COLOR }}>✓</span> Trusted by Brands</span>
            </div>
          </div>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            <button
              ...
              style={{
                background: BRAND_COLOR, color: '#fff', border: 'none', borderRadius: 12,
            ...
            <div style={{ width: '100%', maxWidth: 420, display: 'flex', flexDirection: 'column', gap: 24 }}>
              {profile.projects.map((slot) => (
                <div
                  key={slot.id}
                  style={{
                    background: '#fff', borderRadius: 18, boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
                    padding: 20, display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: 16,
                  }}
                >
                  {/* Platform Icon */}
                  <div style={{ minWidth: 48, minHeight: 48, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: 40, height: 40, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb', border: '1px solid #e5e7eb' }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <div style={{ fontWeight: 600, fontSize: 16, color: TEXT_COLOR, marginBottom: 2 }}>
                      {slot.title && slot.title.length > 100 && !expandedDescriptions[slot.id] ? (
                        <>
                          ...
                          <span style={{ color: BRAND_COLOR, cursor: 'pointer', fontWeight: 600 }}
                    ...
                    {slot.audience_targets && slot.audience_targets.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 2 }}>
                        {slot.audience_targets.map((aud, idx) => (
                          <span key={idx} style={{ background: '#f0fdfa', color: '#0891b2', borderRadius: 12, padding: '4px 12px', fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {slot.topics && slot.topics.length > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 2 }}>
                        {slot.topics.map((topic, idx) => (
                          <span key={idx} style={{ background: '#f3e8ff', color: '#7c3aed', borderRadius: 12, padding: '4px 12px', fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {/* Bid Input */}
                    <form
                      style={{ display: 'flex', flexDirection: 'row', gap: 8, alignItems: 'center', width: '100%', marginTop: 4 }}
                      onSubmit={e => { e.preventDefault(); handleSubmitBid(slot.id); }}
                    >
                      <div style={{ position: 'relative', flex: '1 1 0', minWidth: 0 }}>
                        <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', color: SUBTLE_TEXT, fontWeight: 600, fontSize: 15, zIndex: 1 }}>
                          {getCurrencySymbol(getSlotCurrency(slot))}
                        </span>
                        <input
                          type="number"
                          inputMode="decimal"
                          ...
                          style={{
                            padding: '8px 8px 8px 40px', border: `1.5px solid ${bidErrors[slot.id] ? '#ef4444' : '#e5e7eb'}`,
                            borderRadius: 8, fontSize: 15, outline: 'none', color: TEXT_COLOR,
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: 'none', borderRadius: 8, padding: '10px 18px', fontWeight: 700, fontSize: 15,
                          minWidth: 100,
                        }}
                      >
                        Submit Bid
                    ...
                    {bidErrors[slot.id] && (
                      <div style={{ background: '#fff6f6', color: '#e11d48', fontSize: 13, marginTop: 6, borderRadius: 8, padding: '7px 14px', fontWeight: 500, border: '1px solid #fecaca' }}>
                        {bidErrors[slot.id]}
                      </div>
                    )}
                  </div>
          ...
          {/* Creator CTA */}
          {profile.projects && profile.projects.length > 0 && (
            <div style={{ width: '100%', maxWidth: 420, marginTop: 32, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <a
                href="/register?type=creator"
                style={{
                  display: 'block', background: '#fff', color: BRAND_COLOR, borderRadius: 12,
                  padding: '12px 24px', fontWeight: 600, fontSize: 14, textDecoration: 'none',
                  border: `2px solid ${BRAND_COLOR}`, textAlign: 'center',
                }}
              >
                Are you a creator? Get Your Profile
              </a>
              <div style={{ textAlign: 'center', fontSize: 11, color: SUBTLE_TEXT, marginTop: 6, lineHeight: 1.4 }}>
          ...
          <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '48px 0 0 0' }}>
            <div style={{ color: TEXT_COLOR, fontSize: 16, marginBottom: 12, fontWeight: 600 }}>Share this profile</div>
            <div style={{ fontSize: 12, color: SUBTLE_TEXT, marginBottom: 16, textAlign: 'center', maxWidth: 300 }}>
              ...
              {shareButtons.map((btn) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8, padding: '7px 13px', fontWeight: 600, fontSize: 14, cursor: 'pointer',
                  }}
                >
                  {btn.icon} {btn.label}
                </button>
          ...
          <footer style={{ marginTop: 48, textAlign: 'center', color: SUBTLE_TEXT, fontSize: 14, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <div>
              Powered by{' '}
              <a href="https://newcollab.co" style={{ color: BRAND_COLOR, fontWeight: 600, textDecoration: 'none' }}>Newcollab</a>
            </div>
            <div style={{ marginTop: 2, fontSize: 13, display: 'flex', gap: 12 }}>
        ...
        {showPROfferForm && profile && (
          <PROfferForm
            ...
            onSuccess={() => setShowPROfferForm(false)}
          />
        )}
      </div>
    </LandingPageLayout>
