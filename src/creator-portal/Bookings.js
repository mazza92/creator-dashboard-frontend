...
const Bookings = () => {
  ...
  useEffect(() => {
    ...
    const hasSeenTour = localStorage.getItem('hasSeenBookingsTour');
    if (!hasSeenTour) {
      setRunTour(true);
    }
    setLoading(false);
  ...
  const handleJoyrideCallback = (data) => {
    ...
    if ([STATUS.FINISHED, STATUS.SKIPPED].includes(status)) {
      setRunTour(false);
      localStorage.setItem('hasSeenBookingsTour', 'true');
    }
  };
  ...
  return (
    <Container>
      <Joyride
        ...
        showProgress
        showSkipButton
        steps={tourSteps.bookings}
        styles={tourStyles}
      />

      <Header className="bookings-header">
        <Title level={2} style={{ margin: 0, color: '#1f2937' }}>Bookings</Title>
      ...
      <ContentCard className="booking-filters">
        <Title level={4} style={{ marginBottom: 24, color: '#1f2937' }}>Filters</Title>
        {/* Add your filters here */}
      </ContentCard>

      <ContentCard className="booking-actions">
        <Title level={4} style={{ marginBottom: 24, color: '#1f2937' }}>Actions</Title>
        {/* Add your actions here */}
      </ContentCard>

      <ContentCard className="booking-stats">
